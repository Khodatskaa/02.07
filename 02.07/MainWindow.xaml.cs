...
namespace _02._07
{
    ...
    {
        public List<string> SearchEngines { get; } = new List<string> { "Google", "Bing" };
        ...
        private async Task<List<string>> SearchImagesAsync(string query)
        {
            ...
            if (cmbSearchEngines.SelectedItem != null)
            {
                string selectedEngine = cmbSearchEngines.SelectedItem.ToString().ToLower();
                switch (selectedEngine)
                {
                    case "google":
                        imageUrls = await SearchGoogleImagesAsync(query);
                        break;
                    case "bing":
                        imageUrls = await SearchBingImagesAsync(query);
                        break;
                    default:
                        MessageBox.Show("Unsupported search engine selected.");
                        break;
                }
            }
        ...
        private async Task<List<string>> SearchGoogleImagesAsync(string query)
        {
        ...
        private async Task<List<string>> SearchBingImagesAsync(string query)
        {
            ...
            using (HttpClient client = new HttpClient())
            {
                ...
                return ExtractBingImageUrls(responseBody);
            }
        }
        ...
        private List<string> ExtractBingImageUrls(string html)
        {
            List<string> imageUrls = new List<string>();
